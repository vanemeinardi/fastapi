...
def crea_venta(venta: Ventas) -> dict:
    print("Inside crea_venta")
    db = sesion()
    try:
        # Creating a new instance of the VentasModelo
        ...
        # Handling exceptions, rolling back changes, and returning an error message
        db.rollback()
        print(f'Error al registrar venta: {str(e)}')
        return JSONResponse(content={'mensaje': f'Error al registrar venta: {str(e)}'}, status_code=500)
